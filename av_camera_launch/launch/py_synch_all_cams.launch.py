...
def make_parameters(context):
    ...
    calib_url = 'file:///opt/ros_ws/src/av_camera_launch/config/'
    ...
    cam_parameters['parameter_file'] = PJoin([pd, 'chameleon.yaml'])
    for cam, serial in camera_list.items():
        cam_params = {cam + '.' + k: v for k, v in cam_parameters.items()}
        cam_params[cam + '.serial_number'] = serial
        cam_params[cam + '.camerainfo_url'] = calib_url + cam + '_calib.yaml'
        cam_params[cam + '.frame_id'] = 'camera_' + cam + '_optical'
    ...
    return LaunchDescription(
        [
            ...
            LaunchArg(
                ...
                default_value=['av_camera_launch'],
